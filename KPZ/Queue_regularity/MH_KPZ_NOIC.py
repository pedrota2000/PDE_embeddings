from KPZ_class_NOIC import *

# Define the path
import os
base_dir = os.path.abspath(os.path.dirname(__file__))
plot_path = os.path.join(base_dir, '../Plots/KPZ_regularity_v2/')
...
print(train_generator.get_examples()[0].shape)
...
# Pick the 0 initial condition, in this way we can isolate the non-regular parts. #
    init_cond.append([BundleIBVP1D(t_min=t_min, t_min_val=lambda x: 0.0,
            x_min = x_min*2 ,x_min_val= lambda t: 0,
            x_max = x_max*2 ,x_max_val= lambda t: 0)
    ])
...
for heads in range(solver.n_heads):
    solver.best_nets = solver.best_nets_list[heads]
    h_weights = solver.best_nets[0].head_model.weight
...
body_layers = solver.best_nets[0].H_model.NN
...
# Show solution #
for head in range(solver.n_heads):
    # Move nets to cuda:0 before using them
    device = torch.device("cuda:0")
    for i in range(solver.best_nets.shape[0]):
        solver.best_nets[i] = solver.best_nets[i].to(device)
    solver.best_nets = solver.best_nets_list[head] # Fix the nets to the best nets #
    solver.conditions = solver.all_conditions[:,head]
    #sol = solver.get_residuals(xbest= True)    # Get solution #
    # Select 5 uniformly spaced indices from nu_list
    #nu_indices = np.linspace(0, len(nu_list) - 1, 5, dtype=int)
    u = solver.get_residuals(X,T,N,best=True)
    for idx in range(len(nu_list)):
        plt.figure()
        plt.pcolormesh(X[:, :, idx].cpu().detach().numpy(), 
                       T[:, :, idx].cpu().detach().numpy(), 
                       u[:, :, idx].cpu().detach().numpy(), cmap='rainbow')
        plt.colorbar(label='Residuals')
        plt.xlabel('x')
        plt.ylabel('t')
        plt.title('$\\gamma$ = ' + str(round(nu_val.item(), 3)))
        plt.savefig(plot_path + f'/res_head_{head}_nu_{round(N[0,0,idx].item(),3)}.pdf')
        plt.close()
    plt.show()

...
class MHSolver2D(Solver2D):
    ...
    def __init__(self,  pde_system, conditions_list, all_nets,n_samplings ,theta_min=None, theta_max=None, eq_param_index=(),
                method = 'equally-spaced-noisy', *args, **kwargs):
...
#                 valid_generator ^= Generator1D(32, t_min=r_min[i + 1], t_max=r_max[i + 1], method='equally-spaced')
    ...
    #self.xy_min, self.xy_max = xy_min, xy_max  # Maybe this line is not necessary since it is in r_min & r_max (I think)
       ...
       # )
        ...
        # Fix the lambdas and the alphas for the add loss #
        
        # Get the output dimension of 
        self.nt, self.nx, self.nunu = n_samplings
        self.M = self.all_nets[0,0].H_model.NN[-1].out_features
        self.lamda = 1/2**torch.linspace(1,self.M,self.M)
        self.alphak = torch.tensor([1/2 +1*(k) for k in range(self.M)])
        self.scale = torch.zeros([self.M,self.M])
        for i in range(self.M):
            for j in range(self.M):
                self.scale[i,j] = (self.lamda[i]**self.alphak[j])
        self.scale = self.scale.unsqueeze(0).unsqueeze(0).unsqueeze(0)  # add dimensions for broadcasting
        self.scale = self.scale.repeat(self.nt-1,self.nx-1,self.nunu,1,1)
    ...
    def additional_loss(self, r,f,x):
        # Get the generators # 
        xx,tt,nunu = self.train_generator.get_examples()
        Omega = torch.cat([xx.reshape(-1,1),tt.reshape(-1,1),nunu.reshape(-1,1)],dim = 1)
        H = self.nets[0].H_model(Omega).reshape(self.nt,self.nx,self.nunu,-1)
        new_row = torch.zeros(H.shape[1::])
        H_m = torch.cat([H, new_row.unsqueeze(0)], dim=0)
        H_p = torch.cat([new_row.unsqueeze(0), H], dim=0)
        new_column = torch.zeros(H_m.shape[0], H_m.shape[2],H_m.shape[3])
        H_m = torch.cat([H_m, new_column.unsqueeze(1)], dim=1)
        H_p = torch.cat([new_column.unsqueeze(1),H_p], dim=1)
        inc = H_p - H_m
        inc = inc[1:-1,1:-1,:,:]
        inc = inc.unsqueeze(-2).repeat(1,1,1,self.M,1)   # additional dimension being l
        r = inc/self.scale
        Var_r = torch.var(r,dim = (0,1,2,3),keepdim = True).squeeze()
        add_loss = torch.mean(Var_r)
        # Use a sigmoid to smoothly turn on the loss after 250k epochs
        turn_on = torch.sigmoid(torch.tensor((self.global_epoch - 400_000) / 5_000, dtype=torch.float32))
        if self.lowest_loss is None:
            return  1e-45 * add_loss * turn_on
        else:
            # Set the additional loss to be 10% of the min total loss #
            return 1e-45 * add_loss * turn_on
...
#from MHBundleSolver2D_spareseness import *

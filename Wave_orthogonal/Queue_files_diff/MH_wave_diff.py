from MHBundleSolver2D_wave_orthogonal import *

# Define the path
import os
base_dir = os.path.abspath(os.path.dirname(__file__))
plot_path = os.path.join(base_dir, '../Plots/Wave_25NU_diff_Orthogonal/')
...
for i in range(1):
    init_cond.append([BundleIBVP1D(t_min=t_min, t_min_val=lambda x: init_gauss(x,0.5,1.0),
            x_min = x_min*10 ,x_min_val= lambda t: 0,
            x_max = x_max*10 ,x_max_val= lambda t: 0),
        BundleIBVP1D(t_min=t_min, t_min_val=lambda x: torch.sin(x*torch.pi/5) + 0.2 * torch.cos(x * torch.pi/5 * 1/2) + 2*torch.sin(x*torch.pi/5 * 2),
            x_min = x_min*10 ,x_min_val= lambda t: 0,
            x_max = x_max*10 ,x_max_val= lambda t: 0),
        BundleIBVP1D(t_min=t_min, t_min_val=lambda x: init_N_wave(x,1.0,1.0),
            x_min = x_min*10 ,x_min_val= lambda t: 0,
            x_max = x_max*10 ,x_max_val= lambda t: 0),
        BundleIBVP1D(t_min=t_min, t_min_val=lambda x: -init_gauss(x,0.5,1.0),
            x_min = x_min*10 ,x_min_val= lambda t: 0,
            x_max = x_max*10 ,x_max_val= lambda t: 0),
        BundleIBVP1D(t_min=t_min, t_min_val=lambda x: morlet_wavelet(x, f=0.25, sigma=1.25)+1.0,
            x_min = x_min*10 ,x_min_val= lambda t: 0,
            x_max = x_max*10 ,x_max_val= lambda t: 0)
    ])

from MHBundleSolver2D_orthogonal import *

# Define the path
import os
base_dir = os.path.abspath(os.path.dirname(__file__))
plot_path = os.path.join(base_dir, '../Plots/Burgers_25NU_poly_Orthogonal/')
...
x_min = -5
x_max = +5
number_of_epochs = 150_000  #10_000
...
for i in range(n_init_cond):
    # use a wider amplitude range (can include negatives) and pick distinct basis polynomials
    amplitude = [float(np.random.uniform(-5.0, 5.0)) for _ in range(3)]
    p_list = [p1, p2, p3, p4, p5, p6]
    position = list(np.random.choice(len(p_list), size=3, replace=False))
    init_cond_parameters.append([position[0], position[1], position[2],
                                 amplitude[0], amplitude[1], amplitude[2]])

    # bind values into a local function to avoid closure issues
    def make_pcond(p_list_local, amp_local, pos_local):
        def _pcond(x):
            s = 0.0
            for j in range(3):
                s += amp_local[j] * p_list_local[pos_local[j]](x)
            return s
        return _pcond

    pcond = make_pcond(p_list, amplitude, position)

    init_cond.append(
        BundleIBVP1D(
            t_min=t_min,
            t_min_val=pcond,
            x_min=x_min, x_min_val=lambda t: 0.0,
            x_max=x_max, x_max_val=lambda t: 0.0
        )
    )

print(init_cond_parameters)
init_cond_params_array = np.array(init_cond_parameters).reshape(-1, 6)
np.savetxt(plot_path + '/init_cond_parameters.txt', np.array(init_cond_params_array))

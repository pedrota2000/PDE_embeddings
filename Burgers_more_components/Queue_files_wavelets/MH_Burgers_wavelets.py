...
plot_path = os.path.join(base_dir, '../Plots/Burgers_25NU_wavelets_Orthogonal/')
...
nu_list = torch.logspace(-2, 0, 15)  # List of nu values for the PDEs
...
for i in range(n_init_cond):
    # Generate 20 random shifts in between -0.8 and 0.8 #
    shifts = [np.random.uniform(-0.8, 0.8) for _ in range(20)]
    # Generate 20 random scales in between 0.6 and 1.5 #
    scales = [np.random.uniform(0.8, 1.5) for _ in range(20)]
    # Generate 20 random coefficents in between 0 and 1 #
    coefficients = [np.random.uniform(0.0, 1.0) for _ in range(20)]
    init_cond.append(BundleIBVP1D(t_min=t_min, t_min_val=lambda x, sh=shifts, sc=scales, co=coefficients: sum(co[i] * R_wavelet_POS(x, sc[i], sh[i]) for i in range(20)),
            x_min = x_min ,x_min_val= lambda t: 0,
            x_max = x_max ,x_max_val= lambda t: 0)
    )
    init_cond_parameters.append([shifts, scales, coefficients])

init_cond_params_array = np.array(init_cond_parameters).reshape(-1, 3)
np.savetxt(plot_path + '/init_cond_parameters.txt', init_cond_params_array)

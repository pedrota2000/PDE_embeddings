...
plot_path = os.path.join(base_dir, '../Plots/Burgers_25NU_Fourier_Orthogonal_random/')
...
for i in range(n_init_cond):
    ...
    init_cond_parameters.append((amplitude, frequency))
    # Capture current values with default arguments
    init_cond.append(BundleIBVP1D(
        t_min=t_min, 
        t_min_val=lambda x, amp=amplitude, freq=frequency: init_sine(x, amp, freq),
...
for i in range(n_init_cond):
    amplitude = np.random.uniform(0.1, 2.0)
    frequency = np.random.choice([1, 3, 5])
    init_cond_parameters.append((amplitude, frequency))
    init_cond.append(BundleIBVP1D(
        t_min=t_min, 
        t_min_val=lambda x, amp=amplitude, freq=frequency: init_cos(x, amp, freq),
        x_min=x_min, x_min_val=lambda t: 0,
        x_max=x_max, x_max_val=lambda t: 0
    ))
np.savetxt(plot_path + '/init_cond_parameters.txt', np.array(init_cond_parameters))
print(init_cond)
...
# Warm-up for 1000 epochs (linear increase from 0 to 0.001)
